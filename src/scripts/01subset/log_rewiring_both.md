...
New files: `02run_bootstrap_corr.py` — per-gene SGE array task
...
|2 — corr_array|	0..N_GENES-1|	Stage 1 JID|	16 G|	results/corr/gene_XXXXX.h5|
...
## comparing Route through 01cal_corr_edge.py vs. gene_wise way

Whether it is necessary to have a separate script for gene-wise behaviour.

The two options are not equivalent — there are real costs to routing through 01cal_corr_edge.py.

### The critical issue: module-level imports

`01cal_corr_edge.py:50-51` imports two packages unconditionally at the top:
...
`02run_bootstrap_corr.py:46` imports only from scipy.stats import rankdata — the one thing it actually uses.

### Computation path comparison

|Aspect|	Route through 01cal_corr_edge.py|	02run_bootstrap_corr.py|
|---|---|---|
...
1. Primary Request and Intent:
   - **Initial request**: Review `01cal_corr_edge.py` against Black style, summarize functionality, identify efficiency/readability improvements.
   - **Second request**: Same review for `01subset_bootstrap.py`, plus pipeline-level efficiency thinking and SGE job array coordination between the two scripts. User described the pipeline: create low/high expression subpopulations with bootstrapping, feed subsetted datasets to compute co-expression edges, compare low vs high. User explicitly noted `01cal_corr_edge.py` is slow and needs efficiency improvement for cluster parallel execution.
   - **Third request**: Modify `01subset_bootstrap.py` to use "one dataset per category" HDF5 layout instead of one dataset per gene.
   - **Fourth request**: Add `parse_args()` to `01subset_bootstrap.py`, improve pipeline-level efficiency in `01cal_corr_edge.py`, provide terminal example commands.
   - **Fifth (current) request**: "Please wire bootstrap and calculate co-expression pairs together." — create the missing SGE array-task wrapper (`02run_bootstrap_corr.py`) and SGE submission script (`run_bootstrap_pipeline.sh`).
...
2. Key Technical Concepts:
   ...
   - **Block-tiling**: `01cal_corr_edge.py` tiles `Z @ Z.T` into `(block_size × block_size)` chunks to bound peak memory. Not needed for bootstrap subsets because `k_resample` is small.
...
3. Files and Code Sections:
   ...
   - **`src/scripts/01subset/01subset_bootstrap.py`** (heavily modified)
   ...
   - **`src/scripts/10spearman_corr/01cal_corr_edge.py`** (modified for pipeline efficiency)
   ...
   - **`src/SGE_scripts/SGE_template`** (read-only, reference)
   ...
   - **`src/scripts/10spearman_corr/02run_bootstrap_corr.py`** — TO BE CREATED (pending)
   - **`src/SGE_scripts/run_bootstrap_pipeline.sh`** — TO BE CREATED (pending)
...
7. Pending Tasks:
   - **Write `src/scripts/10spearman_corr/02run_bootstrap_corr.py`**: SGE array-task Python script. One job per gene. Reads bootstrap indices from shared HDF5, column-subsets expression, computes `rank_zscore_rows → matmul → triu extraction` per replicate (no block-tiling needed), writes one HDF5 per gene with `low/base`, `low/boot`, `high/base`, `high/boot`. Gene ID from `$SGE_TASK_ID` or `--gene-id`. `--toy` uses same 5×50 matrix as bootstrap `--toy`.
   - **Write `src/SGE_scripts/run_bootstrap_pipeline.sh`**: Two-stage SGE submission. Stage 1 single job → `bootstrap_indices.h5`. Stage 2 job array `-t 0-N` with `-hold_jid` on Stage 1 → `corr/gene_XXXXX.h5`. Follows repo SGE template conventions.
...
8. Current Work:
   The immediate work before the summary request was writing the two files that wire the pipeline together. TodoWrite was set up with three tasks; task 1 ("Write 02run_bootstrap_corr.py") was marked `in_progress`. The design was fully worked out:
   - `rank_zscore_rows` defined locally in the new script (avoids `importlib` for the digit-prefixed `01cal_corr_edge.py` filename and avoids pulling in heavy `statsmodels` import)
...
9. Optional Next Step:
   ...
   Immediate next action: Write `src/scripts/10spearman_corr/02run_bootstrap_corr.py` with the design outlined in §8, then write `src/SGE_scripts/run_bootstrap_pipeline.sh`, then run the end-to-end toy test (bootstrap `--toy` → corr `--toy --gene-id 0` through 4 → verify output).
