...
#
# USAGE — interactive: set the PARAMETERS block below, then run:
#   Rscript tmp/run_gamlss_quintile_analysis.R
#
# USAGE — CLI (positional args override the defaults below):
#   Rscript tmp/run_gamlss_quintile_analysis.R <focal_gene> [n_test_genes] [condition_val]
#     focal_gene    gene ID string                           (required)
#     n_test_genes  integer for test mode; NULL for full run (default: 1)
#     condition_val 1=CT  2=HS                              (default: 1)
#
...
library(glue)
source("tmp/tools_gamlss.R")
source("tmp/gamlss_dvgdeg_core.R")
source("tmp/preprocess_quintile.R")
