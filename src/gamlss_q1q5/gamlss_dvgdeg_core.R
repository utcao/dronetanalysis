...
run_gamlss_dvgdeg <- function(
  ...
  classify <- function(q_mean, q_var) {
    dplyr::case_when(
      q_mean <  fdr_threshold & q_var >= fdr_threshold ~ "DEG",
      q_mean >= fdr_threshold & q_var <  fdr_threshold ~ "DVG",
  ...
  result_list <- foreach(trt = trt_groups) %do% {
    ...
    result_dt <- data.table(
      ...
      class_single    = classify(q_mu_single, q_sigma_single),
      class_modelcomp = classify(q_mu, q_sigma)
    )
